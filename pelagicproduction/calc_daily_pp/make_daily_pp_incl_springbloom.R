...
# --- Config ---
cfg <- list(
  root_path = "pelagicproduction\\calc_daily_pp\\",
...
dir.create(file.path(cfg$root_path, cfg$out_dir), showWarnings = FALSE, recursive = TRUE)
dir.create(file.path(cfg$root_path, cfg$fig_dir), showWarnings = FALSE, recursive = TRUE)
...
pp <- readr::read_csv(file.path(cfg$input_path, cfg$file_pp), show_col_types = FALSE) %>%
  # janitor::clean_names() %>%
...
ggsave(file.path(cfg$fig_dir, "observations.png"), width = 12, height = 9)
...
ggsave(file.path(cfg$fig_dir, "observation_loess.png"), width = 12, height = 9)
